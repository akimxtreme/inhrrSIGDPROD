...
<title>Sistema de Gestión Documental de la Planta Productora de Vacunas - Formularios</title>
<!-- InstanceEndEditable -->
...
echo '<h1 class="tituto_tipo_documento">Formularios (FOR)</h1>';	
echo '</div>';

echo '<div class="ordenar">';
// <><><><><><><><><><><><> Formulario de Búsqueda Rápida <><><><><><><><><><><><><><><><><><><>
formulario_inicio ("Búsqueda de Formularios","formulario","busqueda","","POST","buscar(this);","","","Búsqueda Rápida","");
...
echo '</div>';
			...
			<th class="formulario" id="titulo_documento" style="width:44%;">Título del Documento</th>
			<!--<th class="formulario" id="numero">Rev</th>-->
			<th class="formulario" id="unidad">Unidad Documentación</th>
			<th class="formulario" id="evaluar">Asociacion(es) de PON a FOR</th>
			</tr>';
...
if($busqueda_por == 'busqueda_por'){
	...
	$sql = "SELECT DISTINCT * FROM inf_codigo WHERE tipo_doc='FOR' AND $buscar_por = '$ingrese'";
	$seleccion=mysql_query($sql,$conexion);
 	$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						...
						<td class="titulo_documento" style="width:44%;">'. $campo_6 .'</td>
						<!--<td class="numero">'. $campo_4 .'</td>-->
						<td class="unidad">'. $unidad_bd .'</td>
						<td class="evaluar">
							<form action="evaluacion.php" method="post" name="control_documentos" id="">
							<input type="hidden" name="cod_doc" id="" value="'. $campo_1 .'" />
							<input type="hidden" name="rev" id="" value="'. $campo_4 .'" />
							<button type="submit" name="evaluar" id="" value="evaluar">Asociación de PON</button>
							</form>
						</td>
						</tr>';
...
include ('conexion.php');
$sql = 'SELECT DISTINCT * FROM inf_codigo WHERE tipo_doc="FOR" LIMIT 15';
$seleccion=mysql_query($sql,$conexion);
$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						$campo_1=$row["cod_doc"];
						...
						<td class="unidad">'. $unidad_bd .'</td>
						<td class="evaluar">
							<form action="asoc_pon_a_for.php" method="post" name="asociar_pon" id="">
							<input type="hidden" name="cod_doc" id="" value="'. $campo_1 .'" />
							<input type="hidden" name="rev" id="" value="'. $campo_4 .'" />
							<button type="submit" name="asociar" id="" value="asociar">Asociación de PON</button>
							</form>
						</td>
						</tr>';
