...
<title>Sistema de Gestión Documental de la Planta Productora de Vacunas - Revaluación del Documento</title>
<!-- InstanceEndEditable -->
...
echo '<h1 class="tituto_tipo_documento">Revaluación del Documento</h1>';	
echo '</div>';

echo '<div class="ordenar">';
// <><><><><><><><><><><><> Formulario de Búsqueda Rápida <><><><><><><><><><><><><><><><><><><>
formulario_inicio ("Búsqueda de Documentos Evaluados No Aprobados","formulario","busqueda","","POST","buscar(this);","","","Búsqueda Rápida","");
...
echo '</div>';
			...
			<th class="formulario" id="evaluar">Documentos a Revaluar</th>
			</tr>';
...
if($busqueda_por == 'busqueda_por'){
	...
	$sql = "SELECT * FROM inf_codigo WHERE $buscar_por = '$ingrese' AND estatus='666'";
	$seleccion=mysql_query($sql,$conexion);
 	$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						...
						<td class="evaluar">
							<form action="evaluacion.php" method="post" name="revaluacion_documentos" id="">
							<input type="hidden" name="cod_doc" id="" value="'. $campo_1 .'" />
							<input type="hidden" name="rev" id="" value="'. $campo_4 .'" />
							<button type="submit" name="evaluar" id="" value="evaluar">Revaluar Documento</button>
							</form>
						</td>
						</tr>';
...
include ('conexion.php');
$sql = 'SELECT * FROM inf_codigo WHERE estatus="666" LIMIT 5';
$seleccion=mysql_query($sql,$conexion);
$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						$campo_1=$row["cod_doc"];
