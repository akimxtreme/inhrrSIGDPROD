...
<title>Sistema de Gestión Documental de la Planta Productora de Vacunas - Búsqueda de Documentos para su Devolución</title>
<!-- InstanceEndEditable -->
...
echo '<h1 class="tituto_tipo_documento">Devolución de Documentos</h1>';	
echo '</div>';

echo '<div class="ordenar">';
// <><><><><><><><><><><><> Formulario de Búsqueda Rápida <><><><><><><><><><><><><><><><><><><>
formulario_inicio ("Devolución de Documentos","formulario","busqueda","","POST","buscar(this);","","","Búsqueda Rápida","");
...
echo '</div>';
			...
			<th class="formulario" id="evaluar">Devolución de Documentos</th>
			</tr>';
...
if($busqueda_por == 'busqueda_por'){
	...
	$sql = "SELECT * FROM inf_codigo WHERE $buscar_por = '$ingrese' AND estatus='2'";
	$seleccion=mysql_query($sql,$conexion);
 	$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						...
						<td class="evaluar">
							<form action="devolucion.php" method="post" name="devolucion_documentos" id="">
							<input type="hidden" name="cod_doc" id="" value="'. $campo_1 .'" />
							<input type="hidden" name="rev" id="" value="'. $campo_4 .'" />
							<button type="submit" name="recibir" id="" value="recibir">Devolución del Documento</button>
							</form>
						</td>
						</tr>';
...
include ('conexion.php');
$sql = 'SELECT * FROM inf_codigo WHERE estatus="2" LIMIT 5';
$seleccion=mysql_query($sql,$conexion);
$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						$campo_1=$row["cod_doc"];
