...
<title>Buscar Usuario - (Reinicio de Contraseña)</title>
<!-- InstanceEndEditable -->
...
echo '<h1 class="tituto_tipo_documento">Búsqueda de Usuario - (Reinicio de Contraseña)</h1>';	
echo '</div>';
...
// Formulario de Solicitud de Códigos
echo '<div class="ordenar">';
formulario_inicio ("Buscar Usuario - (Reinicio de Contraseña)","formulario","busqueda","","POST","consultaUsuario(this);","","","","");
...
if($buscar == 'buscar'){
	...
	$sql = "SELECT * FROM usuario WHERE $buscar_usuario LIKE '%$campo%' AND (privilegio='777' OR privilegio='2' OR privilegio='1')  AND cod_unidad!='IF' AND usuario!='$usuario'";
	$seleccion=mysql_query($sql,$conexion);
 	$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						...
						<td class="codigo">
							<form action="reinicio_usuario.php" method="post" name="reinicio_usuario" id="">
							<input type="hidden" name="usuario_reinicio" id="" value="'. $campo_1 .'" />
							<button type="submit" name="reinicio_usuario" id="" value="reinicio_usuario">Ir a Reiniciar</button>
							</form>
						</td>
						</tr>';
...
include ('conexion.php');
$sql = "SELECT * FROM usuario WHERE (privilegio='777' OR privilegio='2' OR privilegio='1')  AND cod_unidad!='IF' AND usuario!='$usuario'";
$seleccion=mysql_query($sql,$conexion);
$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						$campo_1=$row['usuario'];
