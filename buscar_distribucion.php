...
<title>Sistema de Gestión Documental de la Planta Productora de Vacunas - Búsqueda de Documentos a Distribuir</title>
<!-- InstanceEndEditable -->
...
echo '<h1 class="tituto_tipo_documento">Búsqueda de Documentos a Distribuir</h1>';	
echo '</div>';

echo '<div class="ordenar">';
// <><><><><><><><><><><><> Formulario de Búsqueda Rápida <><><><><><><><><><><><><><><><><><><>
formulario_inicio ("Búsqueda de Documentos a Distribuir","formulario","busqueda","","POST","buscar(this);","","","Búsqueda Rápida","");
...
echo '</div>';
			...
			<th class="formulario" id="evaluar">Distribución de Documentos</th>
			</tr>';
...
if($busqueda_por == 'busqueda_por'){
	...
	$sql = "SELECT * FROM inf_codigo WHERE $buscar_por = '$ingrese' AND estatus='3'";
	$seleccion=mysql_query($sql,$conexion);
 	$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						...
						<td class="evaluar">
							<form action="distribucion.php" method="post" name="distribucion_documentos" id="">
							<input type="hidden" name="cod_doc" id="" value="'. $campo_1 .'" />
							<input type="hidden" name="rev" id="" value="'. $campo_4 .'" />
							<button type="submit" name="recibir" id="" value="recibir">Distribución</button>
							</form>
						</td>
						</tr>';
...
include ('conexion.php');
$sql = 'SELECT * FROM inf_codigo WHERE estatus="3" LIMIT 5';
$seleccion=mysql_query($sql,$conexion);
$cantidad_de_registros = 1;
				while ($row = mysql_fetch_array($seleccion)){
						$campo_1=$row["cod_doc"];
